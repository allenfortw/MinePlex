  ...
  implements IThrown
{
  ...
  public void Skill(Player player, int level)
  {
    ...
    this.Factory.Projectile().AddThrow(item, player, this, System.currentTimeMillis() + 4000L, true, false, false, 
      Sound.FIZZ, 0.6F, 1.6F, null, 0, UpdateType.FASTEST, 1.0D);
  ...
  public void Strike(LivingEntity target, ProjectileUser data)
  {
    ...
    HashMap<Player, Double> hit = UtilPlayer.getInRadius(data.GetThrown().getLocation(), 6.0D);
    for (Player cur : hit.keySet())
    {

      this.Factory.Condition().Factory().Lightning(GetName(), cur, player, 2, 1.0D, false, true);
    }
    ...
    for (Player cur : hit.keySet())
    {
      ...
      this.Factory.Condition().Factory().Shock(GetName(), cur, player, 2.0D, false, true);
      this.Factory.Condition().Factory().Slow(GetName(), cur, player, 2.0D, 0, false, true, true, true);
      this.Factory.Condition().Factory().Vulnerable(GetName(), cur, data.GetThrower(), 2.0D, 0, false, true, true);
    }
  }

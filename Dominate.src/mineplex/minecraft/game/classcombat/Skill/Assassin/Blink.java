...
public class Blink extends SkillActive
{
  ...
  public boolean CustomCheck(Player player, int level)
  {
    ...
    if ((this._loc.containsKey(player)) && (this._blinkTime.containsKey(player)) && 
      (!mineplex.core.common.util.UtilTime.elapsed(((Long)this._blinkTime.get(player)).longValue(), 5000L)))
    {
  ...
  public void Skill(Player player, int level)
  {
    ...
    this.Factory.Condition().SetIndicatorVisibility(player, true);
  ...
  public void Deblink(Player player, int level)
  {
    ...
    while (!done)
    {
      Vector vec = mineplex.core.common.util.UtilAlg.getTrajectory(player.getLocation(), 

...
public class Stealth extends Skill
{
  ...
  public void Crouch(PlayerDropItemEvent event)
  {
    ...
    if (!this._active.remove(player))
    {
      ...
      if (!mineplex.core.common.util.UtilTime.elapsed(this.Factory.Combat().Get(player).LastCombat, 4000L))
      {
  ...
  public void Add(Player player)
  {
    ...
    this.Factory.Condition().Factory().Slow(GetName(), player, player, 120000.0D, 2, false, false, false, true);
    this.Factory.Condition().Factory().Vulnerable(GetName(), player, player, 120000.0D, 3, false, true, true);
    

    player.getWorld().playSound(player.getLocation(), org.bukkit.Sound.BREATH, 0.5F, 0.5F);
  }
        ...
        if (this._active.contains(cur))
          for (Player other : cur.getWorld().getPlayers())
          {
            if (!other.equals(cur))
            {
              ...
              if (UtilMath.offset(cur, other) <= 3.0D)
              {
  ...
  public void Energy(UpdateEvent event)
  {
    ...
    for (Player cur : GetUsers())
    {
      if (this._active.contains(cur))
      {
        ...
        else if (!this.Factory.Energy().Use(cur, GetName(), 0.75D, true, false))
        {
          Remove(cur, null);
        }
      }

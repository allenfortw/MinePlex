...
public class Needler extends SkillActive
{
  ...
  {
    for (Player cur : GetUsers())
      if (!this._stored.containsKey(cur))
      {
        this._stored.put(cur, Integer.valueOf(0));



      }
      else if ((!this._fired.containsKey(cur)) || 
        (UtilTime.elapsed(((Long)this._fired.get(cur)).longValue(), 1000 - GetLevel(cur) * 50)))
      {

        int charges = ((Integer)this._stored.get(cur)).intValue();
        ...
        if (charges < 3 + 1 * GetLevel(cur))
        {
          ...
          if (Recharge.Instance.use(cur, GetName() + " Recharge", 100 - GetLevel(cur) * 10, false))
          {
            ...
            this._stored.put(cur, Integer.valueOf(charges));

...
public class DefensiveStance extends SkillActive
{
	...
	public void Damager(CustomDamageEvent event)
	{
		...
		if (UtilTime.elapsed(_useTime.get(damager), 500))
			return;
	...
	public void Damagee(CustomDamageEvent event)
	{
		...
		if (damagee.getLocation().getDirection().subtract(from).length() > 0.6)
			return;

		//Damage
		//event.SetKnockback(false);
		event.SetCancelled(GetName() + " Defense");
		//event.AddMod(damagee.getName(), GetName(), -5, true);

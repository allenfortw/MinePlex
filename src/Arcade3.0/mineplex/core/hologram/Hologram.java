...
public class Hologram
{
	...
	private void makeSpawnPackets()
	{
		...
		if (_entityIds.size() < _hologramText.length)
		{
			...
			{
				_entityIds.add(new HashMap.SimpleEntry<Integer, Integer>(UtilEnt.getNewEntityId(), UtilEnt.getNewEntityId()));
			}
		}
		else
		{
		...
		if (isInUse())
		{
			...
			for (int i = 0; i < Math.max(_hologramText.length, newText.length); i++)
			{
				...
				if (i >= _hologramText.length)
				{
					...
					Entry<Integer, Integer> entry = new HashMap.SimpleEntry<Integer, Integer>(UtilEnt.getNewEntityId(), UtilEnt.getNewEntityId());

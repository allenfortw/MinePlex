...
{
	public Menu(CosmeticManager plugin, CosmeticShop shop, CoreClientManager clientManager, Player player)
	{
		super(plugin, shop, clientManager, "Inventory", player);
	...
	{
		getShop().openPageForPlayer(player, new CostumePage(getPlugin(), getShop(), getClientManager(), "Costumes", player));
	}

	public void openMusic(Player player)
	{
		getShop().openPageForPlayer(player, new MusicPage(getPlugin(), getShop(), getClientManager(), "Music", player));
	}
}

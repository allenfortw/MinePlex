...
public class GameLobbyManager implements Listener, IPacketHandler
{
	...
	public GameLobbyManager(ArcadeManager manager, PacketHandler packetHandler)
	{
		...
		_serverName = Bukkit.getServerName();
...
//			WriteKitLine("Kits", 1, 159, (byte)4);
	...
	private String GetKitCustomName(Player player, Game game, LobbyEnt ent)
	{
		...
		if (!player.isOnline() || client == null)
			return entityName;
		...
		if (ent.GetKit().GetAvailability() == KitAvailability.Free || 										//Free
			...
			(ent.GetKit().GetAvailability() == KitAvailability.Achievement ||			//Green
			Manager.GetClients().Get(player).GetRank().Has(Rank.MAPDEV) ||									//STAFF ||					//Single Ultra (Old)
	...
	{		
		...
		boolean rankIsUltra = !Manager.GetClients().Get(player).GetRank().Has(Rank.ULTRA);

...
public class OptionsPage extends ShopPageBase<ArcadeManager, OptionsShop>
{
	...
	public OptionsPage(Build game, ArcadeManager plugin, OptionsShop shop, CoreClientManager clientManager, Player player)
	{
		super(plugin, shop, clientManager, "Options", player, 9);
		_game = game;
		buildPage();
	}
	...
	protected void buildPage()
	{
		...
		addButton(0, ground, new IButton()
		{
			...
			{
				getShop().openPageForPlayer(player, new GroundPage(_game, getPlugin(), getShop(), getClientManager(), player));
			}
		});
		...
		addButton(4, particles, new IButton()
		{
			...
			public void onClick(Player player, ClickType clickType)
			{
				getShop().openPageForPlayer(player, new ParticlesPage(_game, getPlugin(), getShop(), getClientManager(), player));
			}
		});
		...
		addButton(6, weather, new IButton()
		{
			...
			public void onClick(Player player, ClickType clickType)
			{
				getShop().openPageForPlayer(player, new WeatherPage(_game, getPlugin(), getShop(), getClientManager(), player));
			}
		});
		...
		addButton(8, time, new IButton()
		{
			...
			public void onClick(Player player, ClickType clickType)
			{
				getShop().openPageForPlayer(player, new TimePage(_game, getPlugin(), getShop(), getClientManager(), player));
			}
		});
	}
}

...
public class GroundPage extends ShopPageBase<ArcadeManager, OptionsShop>
{
	...
	public GroundPage(Build game, ArcadeManager plugin, OptionsShop shop, CoreClientManager clientManager, Player player)
	{
		super(plugin, shop, clientManager, "Change Ground", player, 9 * 4);
		_game = game;
		buildPage();
	}
	...
	protected void buildPage()
	{
		...
		addButton((9 * 3) + 4, new ShopItem(Material.BED, C.cGray + " \u21FD Go Back", new String[]{}, 1, false), new IButton()
		{
			...
			public void onClick(Player player, ClickType clickType)
			{
				getShop().openPageForPlayer(player, new OptionsPage(_game, getPlugin(), getShop(), getClientManager(), player));
			}
		});
	}
}
